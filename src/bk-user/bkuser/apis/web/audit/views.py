...
class AuditRecordListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if params.get("operator"):
            filters &= Q(creator__icontains=params["operator"])
        ...
        if params.get("object_name"):
            filters &= Q(extras__name__icontains=params["object_name"])

        return OperationAuditRecord.objects.filter(filters)

    def get_serializer_context(self):
        context = super().get_serializer_context()
        context["operation_choices"] = dict(OperationEnum.get_choices())
        context["object_type_choices"] = dict(ObjectTypeEnum.get_choices())
        return context

    @swagger_auto_schema(
        tags=["audit"],
        operation_description="操作审计记录列表",
