...
class OperationAuditRecordListInputSerializer(serializers.Serializer):
    ...
    operation = serializers.ChoiceField(help_text="操作类型", choices=OperationEnum.get_choices(), required=False)
...
class OperationAuditRecordListOutputSerializer(serializers.Serializer):
    ...
    operation = serializers.SerializerMethodField(help_text="操作类型")
    object_type = serializers.SerializerMethodField(help_text="操作对象类型")
    object_name = serializers.SerializerMethodField(help_text="操作对象名称", required=False)
    created_at = serializers.DateTimeField(help_text="操作时间")

    def get_object_name(self, obj):
        # 从 extras 字段中提取 name 值
        return obj.extras.get("name")

    def get_operation(self, obj):
        # 从 operation_choices 字典中提取 operation 对应的中文标识
        return self.context["operation_choices"][obj.operation]

    def get_object_type(self, obj):
        # 从 object_type_choices 字典中提取 object_type 对应的中文标识
        return self.context["object_type_choices"][obj.object_type]
